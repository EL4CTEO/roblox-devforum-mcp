...
test("bugStatus separates a status tag from an accepted answer", () => {
  // Flattened into one word, these two claims are not the same claim: a tag is the forum
  // saying so, has_accepted_answer is whoever opened the thread clicking Solution. Printing
  // the second as "[solved]" under a header promising staff triage told the caller Roblox
  // had confirmed a report nobody at Roblox had touched.
  assert.deepEqual(bugStatus({ id: 1, title: "a", tags: ["confirmed", "datastore"] }), {
    label: "confirmed",
    fromTag: true,
  });
  assert.deepEqual(bugStatus({ id: 1, title: "a", tags: ["cannot-reproduce"] }), {
    label: "cannot-reproduce",
    fromTag: true,
  });
  assert.deepEqual(bugStatus({ id: 1, title: "a", has_accepted_answer: true }), {
    label: "answered",
    fromTag: false,
  });
...
test("replyCount reports thread length, not Discourse's direct-reply count", async () => {
  const { replyCount } = await import("../dist/rank.js");
  // Discourse counts only posts replying to post #1: topic 583926 has 11 posts and reports
  // reply_count 6, and 4052968 has one reply and reports zero.
  assert.equal(replyCount({ id: 1, title: "a", posts_count: 11, reply_count: 6 }), 10);
  assert.equal(replyCount({ id: 2, title: "a", posts_count: 2, reply_count: 0 }), 1);
  assert.equal(replyCount({ id: 3, title: "a", posts_count: 1 }), 0);
  // Listings that omit posts_count still have to answer with something.
  assert.equal(replyCount({ id: 4, title: "a", reply_count: 3 }), 3);
  assert.equal(replyCount({ id: 5, title: "a" }), 0);
});

test("plural keeps a count of one from reading as a rendering bug", async () => {
  const { plural } = await import("../dist/format.js");
...
test("renderGuide keeps the title and drops repo bookkeeping", async () => {
  const { renderGuide } = await import("../dist/docs.js");
  // Reading cloud-services/data-stores handed the caller a note written to Roblox's docs
  // team about not moving anchors, paid for out of the caller's token budget.
  const page = renderGuide(
    "---\ntitle: Data stores\ndescription: How to store persistent data.\ncomments: The Creator Hub links to some of the anchors on this page.\n---\nThe `DataStoreService` lets you store data.\n",
  );
  assert.match(page, /^# Data stores/);
  assert.match(page, /How to store persistent data\./);
  assert.match(page, /DataStoreService/);
  assert.ok(!page.includes("comments:"), "front-matter bookkeeping survived");
  assert.ok(!page.includes("---"), "front-matter fence survived");

  // A page with no front matter is returned as it is.
  assert.equal(renderGuide("# Already Markdown\n\nbody"), "# Already Markdown\n\nbody");
});

test("cleanReferenceYaml drops the generated banner and empty fields", async () => {
  const { cleanReferenceYaml } = await import("../dist/docs.js");
  const out = cleanReferenceYaml(
    "# This file is automatically generated.\n# To submit a bug report, see …\nname: Vector3\ntype: datatype\ncode_samples: []\ntags: []\ndeprecation_message: ''\nsummary: |\n  A 3D value.\n",
  );
  assert.match(out, /^name: Vector3/);
  assert.ok(!out.includes("automatically generated"));
  assert.ok(!out.includes("code_samples"));
  assert.ok(!out.includes("deprecation_message"));
  // A field that actually says something stays.
  assert.match(out, /summary: \|\n {2}A 3D value\./);
});

test("referenceSummary lifts the page's own summary out of the YAML", async () => {
  const { referenceSummary } = await import("../dist/docs.js");
  const yaml = "name: DataStoreRequestType\ntype: enum\nsummary: |\n  Indicates the type of data\n  store request being made.\ndescription: |\n  Categorizes operations.\n";
  // The text around a match in a reference page is "name: X type: enum summary: |" — the
  // file's shape, not an answer, and that is what search results used to show.
  assert.equal(referenceSummary(yaml), "Indicates the type of data store request being made.");
  assert.equal(referenceSummary("name: X\ntype: class\n"), undefined);
});

test("nearestNames offers only names close enough to be a real suggestion", async () => {
  const { nearestNames } = await import("../dist/docs.js");
  const members = ["Magnitude", "Unit", "Cross", "Dot", "FuzzyEq", "new"];
  assert.deepEqual(nearestNames("magnitud", members), ["Magnitude"]);
