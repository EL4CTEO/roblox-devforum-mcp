...
export class HttpClient {
    ...
    constructor(deps) {
        ...
        this.breaker = new CircuitBreaker(deps.config.RDFM_CIRCUIT_THRESHOLD, deps.config.RDFM_CIRCUIT_WINDOW_MS, deps.config.RDFM_CIRCUIT_COOLDOWN_MS);
    }
    buildHeaders(json, etag) {
        const headers = json
    ...
    async raw(url, opts = {}) {
        ...
        for (let attempt = 0; attempt <= retries; attempt++) {
            ...
            try {
                ...
                const transient = lastErr.name === "AbortError" ||
                ...
                if (lastErr instanceof CircuitOpenError)
                    throw lastErr;
                this.breaker.recordFailure(host);
                throw lastErr;
            }
    ...
    async getJsonWithFallback(urls) {
        let lastErr;
