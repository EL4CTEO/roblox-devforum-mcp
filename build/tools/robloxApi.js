...
const inputShape = {
    kind: Kind.describe("What to look up: 'class' (full class doc), 'member' (search property/method/event by name), 'hierarchy' (inheritance tree), 'enum' (one enum), 'enums'/'classes' (list)."),
    name: z
        .string()
        .optional()
        .describe("Class or enum name (case-insensitive). Required for kind='class'/'hierarchy'/'enum'."),
    query: z
        .string()
        .optional()
        .describe("Substring match. Required for kind='member', optional filter for 'enums'/'classes'."),
    member_type: z
...
function memberToSummary(m) {
    ...
    if (m.ReturnType?.Name)
        summary.returnType = m.ReturnType.Name;
    if (m.Description)
        summary.description = m.Description;
    return summary;
}
function buildChain(cls, classes) {
...
function findClass(name, classes) {
    return classes.find((c) => c.Name.toLowerCase() === name.toLowerCase());
}
function classToSummary(cls, classes, includeInherited) {
    ...
    if (cls.Superclass)
        summary.superclass = cls.Superclass;
    if (cls.Description)
        summary.description = cls.Description;
    return summary;
}
function renderClass(c, includeInherited) {
    let out = `# ${c.name}\n`;
...
export function register(server, ctx) {
    server.registerTool("roblox_api", {
        title: "Roblox Engine API",
        description: "Inspect the Roblox engine API (classes, members, enums, hierarchy) from Full-API-Dump.json. Replaces 'get_api_docs' + 'search_api_member' + 'get_class_hierarchy' + 'get_enum'.",
        ...
        try {
            ...
            if (input.kind === "class") {
                if (!input.name)
                    return fail(new Error("name is required for kind='class'"));
                const cls = findClass(input.name, dump.Classes);
                if (!cls) {
                    const partials = dump.Classes.filter((c) => c.Name.toLowerCase().includes((input.name ?? "").toLowerCase()))
                        .slice(0, 10)
                        .map((c) => c.Name);
            ...
            if (input.kind === "member") {
                if (!input.query)
                    return fail(new Error("query is required for kind='member'"));
                ...
                for (const cls of dump.Classes) {
                    for (const m of cls.Members ?? []) {
                        if (!m.Name.toLowerCase().includes(q))
                            continue;
                        if (input.member_type && m.MemberType !== input.member_type)
                            continue;
                ...
                const text = found.length
                    ? `Members matching "${input.query}":\n\n${found
                        .map((r) => `• ${r.member.name} (${r.member.kind}) in ${r.className}${r.member.description ? ` — ${r.member.description}` : ""}`)
                        .join("\n")}`
            ...
            if (input.kind === "hierarchy") {
                if (!input.name)
                    return fail(new Error("name is required for kind='hierarchy'"));
                const cls = findClass(input.name, dump.Classes);
                if (!cls)
                    return ok(`Class "${input.name}" not found.`, { kind: "hierarchy" });
                const chain = buildChain(cls, dump.Classes);
                const directSubclasses = dump.Classes.filter((c) => c.Superclass?.toLowerCase() === cls.Name.toLowerCase())
                    .map((c) => c.Name)
                    .sort();
                const lower = chain.map((n) => n.toLowerCase());
                const totalDescendants = dump.Classes.filter((c) => c.Superclass && lower.includes(c.Superclass.toLowerCase())).length;
                const text = [
                    `# ${cls.Name} Hierarchy`,
                    `Chain: ${chain.join(" > ")}`,
            ...
            if (input.kind === "enum") {
                if (!input.name)
                    return fail(new Error("name is required for kind='enum'"));
                const enumName = input.name;
                const match = dump.Enums.find((e) => e.Name.toLowerCase() === enumName.toLowerCase());
                if (!match) {
                    const similar = dump.Enums.filter((e) => e.Name.toLowerCase().includes(enumName.toLowerCase()))
                        .slice(0, 5)
                        .map((e) => e.Name);
            ...
            if (input.kind === "enums") {
                ...
                return ok(`Roblox API Enums (${names.length} total${input.query ? ", filtered" : ""}):\n\n${limited.join(", ")}`, { kind: "enums", apiVersion: dump.Version, list: limited });
            }
            // classes
            ...
            const limited = names.slice(0, input.limit);
            return ok(`Roblox API Classes (${names.length} total${input.query ? ", filtered" : ""}):\n\n${limited.join(", ")}`, { kind: "classes", apiVersion: dump.Version, list: limited });
        }
        catch (e) {
            return fail(e);
        }
...
//# sourceMappingURL=robloxApi.js.map