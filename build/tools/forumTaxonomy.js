...
const inputShape = {
    kind: Kind.describe("What to list: 'categories' (all forum categories), 'tags' (top tags by use), 'category_meta' (one category's metadata)."),
...
export function register(server, ctx) {
    server.registerTool("forum_taxonomy", {
        title: "DevForum Taxonomy",
        description: "List DevForum categories or tags, or fetch metadata for a single category. Replaces 'get_categories' + 'get_tags' + 'get_category_metadata'.",
        ...
        try {
            if (input.kind === "categories") {
                const data = await ctx.http.getJson(`${URLS.devforum}/categories.json`);
                ...
                const text = items
                    .map((c) => `• ${c.name} (ID: ${c.id}, slug: ${c.slug}) — ${c.topicCount} topics${c.description ? `\n  ${c.description}` : ""}`)
                    .join("\n");
            ...
            if (input.kind === "tags") {
                const data = await ctx.http.getJson(`${URLS.devforum}/tags.json`);
            ...
            for (const url of endpoints) {
                try {
                    const raw = await ctx.http.getJson(url);
                    if (!raw)
                        continue;
            ...
            const item = {
                ...
                subcategories: subs.map((s) => {
                    const sub = { id: s.id, name: s.name };
                    if (typeof s.topic_count === "number")
                        sub.topicCount = s.topic_count;
                    return sub;
                }),
            ...
            text += `Slug: ${c.slug ?? ""}\n`;
            if (c.description)
                text += `Description: ${stripHtml(c.description)}\n`;
...
//# sourceMappingURL=forumTaxonomy.js.map