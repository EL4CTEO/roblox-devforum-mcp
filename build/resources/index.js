...
export function registerAllResources(server, ctx) {
    ...
    server.registerResource("roblox-api-classes-index", "roblox-api://classes", {
    ...
    server.registerResource("roblox-api-enums-index", "roblox-api://enums", {
    ...
    // Templated: API class
    server.registerResource("roblox-api-class", new ResourceTemplate("roblox-api://class/{className}", {
        list: undefined,
        complete: {
    ...
    // Templated: API enum
    server.registerResource("roblox-api-enum", new ResourceTemplate("roblox-api://enum/{enumName}", {
        list: undefined,
        complete: {
    ...
    server.registerResource("roblox-luau-stdlib", new ResourceTemplate("roblox-luau://stdlib/{module}", {
        list: undefined,
        complete: {
            module: async (value) => {
                const q = value.toLowerCase();
        ...
        if (!section.found) {
            return textResource(uri.href, `Library "${moduleName}" not found. Available: ${section.available.join(", ")}.`, "text/plain");
        }
        const text = stripHtml(section.text).slice(0, 8000);
        return textResource(uri.href, `# Luau ${moduleName}\n\n${text}`);
    });
    // Templated: DevForum thread
    server.registerResource("roblox-devforum-thread", new ResourceTemplate("roblox-devforum://thread/{topicId}", {
        list: undefined,
    }), {
        ...
        const data = await ctx.http.getJson(`${URLS.devforum}/t/${encodeURIComponent(id)}.json`);
        return jsonResource(uri.href, data);
    });
    // Templated: DevForum category overview
    server.registerResource("roblox-devforum-category", new ResourceTemplate("roblox-devforum://category/{slug}", {
        list: undefined,
    }), {
    ...
    server.registerResource("roblox-news-release", new ResourceTemplate("roblox-news://release/{version}", {
        list: undefined,
    }), {
        ...
        if (!version) {
            return textResource(uri.href, "Version must be a numeric release id (e.g. 674).", "text/plain");
        }
        ...
        if (!item) {
            return textResource(uri.href, `Release notes for version ${version} not found at ${url}.`, "text/plain");
        }
        return textResource(uri.href, renderReleaseNoteMarkdown(item));
    });
    // Templated: Creator Hub doc page
    server.registerResource("roblox-docs-creator", new ResourceTemplate("roblox-docs://creator/{+path}", {
        list: undefined,
    }), {
        title: "Roblox Creator Hub Doc",
        description: "Creator Hub documentation page (path includes slashes, e.g. 'docs/reference/engine/classes/BasePart').",
        mimeType: "text/markdown",
    }, async (uri, vars) => {
