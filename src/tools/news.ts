...
const inputShape = {
  since: z
    ...
    .optional()
    .describe(
      "Lower bound: ISO date (e.g. 2026-04-01) or relative ('7d', '30d', '90d', 'last_release'). Default '30d'."
    ),
  source: Source.default("all").describe(
    "Which feed to query: 'release_notes' (Creator Hub), 'announcements' (DevForum staff posts), 'all'."
  ),
...
interface UpdatesCategoryResponse {
  category_list?: { categories?: { id: number; slug: string }[] };
  topic_list?: DiscourseTopicListResponse["topic_list"];
  users?: DiscourseUser[];
}
...
function relativeOffset(s: string): Date | null {
  const m = /^(\d+)d$/.exec(s);
  if (!m?.[1]) return null;
...
async function resolveSince(
  ...
  fetchLatestReleaseDate: () => Promise<string | null>
): Promise<Date> {
...
function matchesQuery(item: NewsItem, query: string | undefined): boolean {
  if (!query) return true;
  const q = query.toLowerCase();
  return item.title.toLowerCase().includes(q) || item.summary.toLowerCase().includes(q);
}

function withinSince(item: NewsItem, since: Date): boolean {
...
function topicToNewsItem(
  ...
  const author =
    (firstPosterUserId !== undefined ? userMap.get(firstPosterUserId) : undefined) ??
...
function formatNewsList(items: NewsItem[]): string {
  if (items.length === 0) return "No news items found for the selected window.";
  return items
    .map((item) => {
      const date = item.date ? item.date.slice(0, 10) : "unknown";
      if (item.type === "release_note") {
        const sectionLines: string[] = [];
        const labels: [keyof NonNullable<NewsItem["sections"]>, string][] = [
          ["new_features", "New"],
        ...
        for (const [key, label] of labels) {
          ...
          if (arr && arr.length > 0) sectionLines.push(`  ${label}: ${arr.length}`);
        }
        return [
...
export function register(server: McpServer, ctx: AppContext): void {
  const indexCache = new LruCache<string>({ ttlMs: 60 * 60 * 1000, max: 4 });
  const noteCache = new LruCache<string>({ ttlMs: 24 * 60 * 60 * 1000, max: 60 });

  async function getCachedHtml(cache: LruCache<string>, url: string): Promise<string> {
  ...
  async function fetchReleaseNotes(limit: number): Promise<NewsItem[]> {
  ...
  async function latestReleaseDate(): Promise<string | null> {
    try {
  ...
  async function fetchAnnouncements(limit: number): Promise<NewsItem[]> {
    const items: NewsItem[] = [];
    const seenCategoryUrls = new Set<string>();

    async function fetchCategoryFeed(url: string, label: string): Promise<void> {
      if (seenCategoryUrls.has(url)) return;
      seenCategoryUrls.add(url);
      try {
        const data = await ctx.http.getJson<DiscourseTopicListResponse>(url);
        const topics = data.topic_list?.topics ?? [];
        const userMap = buildUserMap(data.users);
        const staffIds = new Set<number>();
        for (const u of data.users ?? []) if (isStaffUser(u)) staffIds.add(u.id);

        for (const t of topics) {
          const opUserId = t.posters?.[0]?.user_id;
          if (opUserId !== undefined && !staffIds.has(opUserId)) continue;
    ...
    try {
      const parent = await ctx.http.getJson<UpdatesCategoryResponse>(URLS.updatesCategory);
      const subs = parent.category_list?.categories ?? [];
      for (const sub of subs) {
        if (sub.id === 26) continue;
  ...
  server.registerTool(
    "roblox_news",
    {
      ...
      description:
        "Aggregates official Roblox news: Creator Hub Release Notes (versioned, structured into New Features / Improvements / Fixes / Removed) and DevForum Announcements posted by Roblox staff. Filterable by date window, source, and query. Use this to know what changed recently before generating Roblox code.",
