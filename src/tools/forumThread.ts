...
import type { DiscourseThreadResponse } from "../types.js";
...
export function register(server: McpServer, ctx: AppContext): void {
  server.registerTool(
    "forum_thread",
    {
      ...
      description:
        "Fetch a DevForum thread: metadata, original post, accepted answer, and (optionally) replies on the given page. Replaces 'get_thread' + 'get_post_replies'.",
    ...
    async (raw): Promise<ReturnType<typeof ok>> => {
      ...
      try {
        ...
        const data = await ctx.http.getJson<DiscourseThreadResponse>(url);
        const stream = data.post_stream?.stream ?? [];
        const posts = data.post_stream?.posts ?? [];
        const first = posts[0];
        const accepted = posts.find((p) => p.accepted_answer === true);

        const truncatePost = (cooked: string): string => {
        ...
        if (acceptedSummary) structured.acceptedAnswer = acceptedSummary;
