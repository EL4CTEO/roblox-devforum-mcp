...
export function registerAllPrompts(server: McpServer): void {
  server.registerPrompt(
    "research-feature",
    {
      ...
      description:
        "Pipeline: look up the relevant API class, find recent DevForum threads (latest + solved), and check open bug reports. Use this when an agent needs deep context on how a Roblox feature works in practice.",
      argsSchema: {
        feature: z
          .string()
          .describe(
            "Class, member, or concept name (e.g. 'Workspace.StreamingEnabled', 'DataStore', 'Pathfinding')."
          ),
      },
    },
    ({ feature }) => ({
  ...
  server.registerPrompt(
    "explain-error",
    {
      ...
      description:
        "Given an error message or symptom, search solved DevForum threads, related bugs, and the relevant API. Designed for an AI agent debugging a Roblox script.",
      argsSchema: {
  ...
  server.registerPrompt(
    "find-implementation-pattern",
    {
      ...
      description:
        "Given a goal, gather Creator Docs guides, top community tutorials, and the relevant API surface so the agent can implement it correctly.",
      argsSchema: {
        goal: z
          .string()
          .describe(
            "What the user wants to build (e.g. 'leaderstats with DataStore persistence', 'matchmaking for round-based game')."
          ),
      },
    },
    ({ goal }) => ({
  ...
  server.registerPrompt(
    "audit-deprecated-api",
    {
      title: "Audit deprecated/changed API",
      description:
        "Check if a Roblox class or member is deprecated, has breaking changes, or has been called out in engine updates / announcements.",
      argsSchema: {
        target: z
          .string()
          .describe(
            "Class or member name to audit (e.g. 'BodyVelocity', 'Workspace:FindPartOnRay')."
          ),
      },
    },
    ({ target }) => ({
      messages: [
        {
          ...
          content: {
            ...
            text: [
              ...
              `2. roblox_news source="all" since="90d" query="${target}" — recent deprecation announcements + release notes.`,
              `3. forum_search category="updates" query="${target}" limit=8 — older engine announcements.`,
              `4. forum_search status="solved" query="${target} deprecated" limit=5 — migration guides.`,
              "",
