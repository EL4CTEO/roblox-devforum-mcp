...
const server = new McpServer({ name: 'roblox-devforum-mcp', version: '1.0.0' });
...
function topicLine(t: any): string {
...
server.registerTool(
  ...
  async ({ username }) => {
    try {
      ...
      const topics = data.topic_list?.topics || [];
      if (!topics.length) return ok(`No recent activity found for user "${username}".`);
...
server.registerTool(
  ...
  async ({ category_id }) => {
    try {
      ...
      if (c.subcategory_ids?.length) text += `Subcategory IDs: ${c.subcategory_ids.join(', ')}\n`;
